...
function SceneSlotMachine_CandyParty:Start()
	...
	--print("Start _Slot ret=",json.encode(ret)," prize=",json.encode(prize))
...
function SceneSlotMachine_CandyParty:StartAll()
	--local self=SceneSlotMachine_CandyParty
	print("init run startAll FN2")
	print("level_key_countdown",self.level_key_countdown)
	print("level",self.level)
	print("--data:",json.encode(self.data))	
	print("--data_Free:",json.encode(self.data_FreeSpin))
	print("bet_coin:",self.bet_coin)
    ...
    print("Lua StartAll --2")
    -- print(self)
    -- print(self.Start)

	-- Main
    local ret, prize = self.Start(self)
    print("Lua StartAll --3")
    print("StartAll ret=",json.encode(ret))
    ...
    if true then
        if #ret[#ret].prize_line ~= 0 then
            ...
            print("Lua StartAll --5-1")
            --	debug.dump (o_ret, "free ret", 10);
            for i = 1, #o_ret do
                ret[#ret + 1] = o_ret[i]
            end
            print("Lua StartAll --5-1-1")
            if cBookKeep then
        ...
        if #ret[#ret].prize_line ~= 0 then
            if cBookKeep then
                ...
                print("Lua StartAll --5-2")
            end
        end
    end

    print("Lua StartAll --6")

    --	debug.dump (ret, "ret", 10);
	_AddResultTop (self, ret)
	print("Lua StartAll --7")
    return ret
end
